...
export default function PlayerBar({
  ...
  const cover_glow = useCoverGlow(currentTrack?.cover ?? null);

  const cover_style = currentTrack?.cover
    ? {
        backgroundImage: `url(${currentTrack.cover})`,
        backgroundSize: "cover",
        backgroundPosition: "center",
        ...(cover_glow ? ({ "--cover-glow": cover_glow } as CSSProperties) : null),
      }
    : undefined;

  const is_repeat_all = repeatMode === "all";
  const is_repeat_one = repeatMode === "one";
  const repeat_label =
    repeatMode === "one"
      ? "Повтор трека"
      : repeatMode === "all"
  ...
  const handle_share_drag_start = (event: DragEvent<HTMLDivElement>) => {
    ...
    event.dataTransfer.setData("text/uri-list", share_url);
    event.dataTransfer.setData("text/plain", share_url);
    event.dataTransfer.effectAllowed = "copyLink";
  };

  const handle_context_menu = (event: MouseEvent<HTMLDivElement>) => {
    if (!currentTrack || currentTrack.isAd) {
      return;
    }
    event.preventDefault();
    event.stopPropagation();
    set_context_menu({ position: { x: event.clientX, y: event.clientY } });
  };
    ...
    <>
      <div className="player">
        <div
          ...
          onDragStart={handle_share_drag_start}
          onContextMenu={handle_context_menu}
          title={currentTrack ? "Перетащите, чтобы поделиться" : undefined}
        >
          <div
            className={`player-cover${cover_glow ? " cover-glow" : ""}`}
            style={cover_style}
          />
        ...
        <div className="player-extras">
          ...
          <button
            className={`control repeat-toggle${is_repeat_all ? " active" : ""}${
              is_repeat_one ? " repeat-one" : ""
            }`}
            type="button"
      ...
      <TrackContextMenu
        open={Boolean(context_menu && currentTrack)}
        position={context_menu?.position ?? null}
        track={
          currentTrack
            ? {
                id: currentTrack.id,
